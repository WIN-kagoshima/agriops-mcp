...
export function registerGetEstatStats(server: McpServer, deps: Deps): void {
  server.registerTool(
    meta.name,
    {
    ...
    async (raw: unknown) => {
      ...
      try {
        ...
        const headers = [
          ...axesList.map((a) => {
            const cls = result.classInfo.find((c) => c.id === a);
            return cls?.name ?? a;
          }),
          "値",
        ];
        ...
        const dataRows = displayValues.map((v) => {
