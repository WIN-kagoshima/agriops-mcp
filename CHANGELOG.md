...
- **`tests/conformance/anonymous-access.test.ts`**: asserts `/livez`, `/readyz`, `.well-known/mcp-server.json`, `initialize`, and `tools/list` all succeed over the real `startHttp` transport with zero `Authorization` header — the code-level contract behind the anonymous public Cloud Run deployment.
